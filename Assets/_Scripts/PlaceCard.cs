...
public class PlaceCard : MonoBehaviour
{
    ...
    public void PlaceOnSlot(GameObject g, int q)
    {
        PlaceOnSlot(g);
        targetPos = point.position + new Vector3(0f, 0f, space * q);
        //Debug.Log("target is " + targetPos);
    }
    ...
    void Update()
    {
        ...
        if (Vector3.Distance(targetPos, go.transform.position) <= 0.1f)
        {
